...
import { Menu, X, Home, User, Code, BookOpen, Mail, Github, Linkedin, Twitter } from "lucide-react";

export function EnhancedGamingHeader() {
          ...
          <div className="p-6 border-b border-gray-800">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-white">GamerPortfolio</h3>
                  <div className="text-xs text-cyan-400">Level 99 Developer</div>
                </div>
              </div>
              <button 
              ...
              <div className="flex space-x-4">
                {socialLinks.map((social) => (
                  <a
                    key={social.name}
                    href={social.href}
                    className="w-10 h-10 bg-gray-800/50 border border-gray-700 rounded-lg flex items-center justify-center text-gray-400 hover:text-white hover:border-purple-500/50 transition-all"
                  >
                    <social.icon size={18} />
                  </a>
                ))}
